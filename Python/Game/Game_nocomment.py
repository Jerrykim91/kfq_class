...
0 ~ 50사이의 숫자를 넣어봐~ \n
    ''',
    'INPUT_EMPTY':'값이 정확 하지 않아; 다시 확인 해줄래?',
    'INPUT_NOT_NUM':'숫자가 아닌걸??',
    'out_of_bound':'응? 숫자가 0~50사이가 아닌데?\n0~50 사이로 다시 부탁해~',
    'check_err01':'''
{name}...너의 숫자가 많아..
{name}의 현재까지 도전 횟수는 {cnt}번 이야.
    ''',
    'check_err02':'''
{name}...너의 숫자가 부족해....
{name}의 현재까지 도전 횟수는 {cnt}번이야.
    ''',
    'check_success':'''
{name}:{0}, 휘니:{1} 정답입니당.
...
game_title = "UP_DOWN GMAE // LET's GET THE START"
cell_amt = 40 
form = '={0:^%s}=' %(cell_amt-2)
...
while game_run:
    ...
    while True:     
        ...
        if gamer_num > ai_num:
            ...
            print(game_txtEx['check_success']
            .format(
                gamer_num,
                ai_num,
                score=(110-try_count*10),
